...
def writeCSV(list):
    ...
    if os.path.isfile("musinsa_review.csv"):
        ...
        with open('musinsa_review_onepiece.csv', 'w', newline='', encoding='utf-8-sig') as f_object:
            writer_object = writer(f_object)
            writer_object.writerow(list_title)
            f_object.close()

    with open('musinsa_review_onepiece.csv', 'a', newline='', encoding='utf-8-sig') as f_object:
        writer_object = writer(f_object)
        for data in list:
...
def get_link():
    global headers
    link = []
    goodsList = pd.read_csv("C:\\Users\\SSAFY\\Desktop\\ssafy\\DA_pjt\\S06P22E202\\data\\DataCrawling\\정재호\\onepiece_unique_id.csv")
    goodsList = list(goodsList)
    page = 1
    for goodsNo in goodsList:
        for baseUrl in baseUrlList:
            url = baseUrl + "?" + "page=" + str(page) + "&goodsNo=" + goodsNo
            ...
            try:
                ...
                for pages in range(1, lastPage + 1):
                    url = baseUrl + "?" + "page=" + str(pages) + "&goodsNo=" + goodsNo
                    link.append(url)
            except:
                pass
    print('link 끝')
