...
def writeCSV(list):
    ...
    if os.path.isfile("musinsa_review_top3_3.csv"):
        pass
    else:
        with open('musinsa_review_top3_3.csv', 'w', newline='', encoding='utf-8-sig') as f_object:
            writer_object = writer(f_object)
            writer_object.writerow(list_title)
            f_object.close()

    with open('musinsa_review_top3_3.csv', 'a', newline='', encoding='utf-8-sig') as f_object:
        writer_object = writer(f_object)
        for data in list:
...
def get_goodsNo():
    global headers
    link = []
    goodsList = pd.read_csv("./unique_id_top3_3.csv")
